...
def render_auth_rules(vsys_root: Element | None, console, grep=None) -> None:
    table = Table(title="Authentication Rules", box=box.ROUNDED, show_lines=True,
    ...
    table.add_column("Src User", min_width=12)
    table.add_column("Service", min_width=10)
    table.add_column("Auth Enforcement", min_width=18)
    table.add_column("Log Setting", min_width=12)
    ...
    if container is None:
        console.print("[dim]No authentication rules found.[/dim]")
        return
    ...
    for i, rule in enumerate(container.findall("entry"), 1):
        ...
        src_user = fmt_members(get_members(rule, "source-user/member"))
        ...
        if not grep_row(grep, name, from_zone, to_zone, src_addr, dst_addr,
                        src_user, service, auth_enforcement, log_setting):
            continue

        name_str = f"[dim]{name} [disabled][/dim]" if disabled else name
        table.add_row(str(i), name_str, fmt_any(from_zone), fmt_any(to_zone),
                      fmt_any(src_addr), fmt_any(dst_addr), fmt_any(src_user),
                      fmt_any(service), auth_enforcement, log_setting)
        added += 1
